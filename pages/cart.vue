      ...
      >
        ...
        >
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <NuxtLink to="/">
              <UButton color="cyan">Homepage</UButton>
            </NuxtLink>
            <div class="w-fit mx-auto grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 justify-items-center justify-center gap-y-20 gap-x-14 mt-10 mb-5">
              <UCard
                ui="{shadow: 'shadow'}"
                class="w-72"
                v-for="product in productStore.cart" :key="product.id">
...
<script setup lang="ts">
  ...
  function removeFromCart(product: any) {
    state.productsToCart = productStore.cart
    state.productsToCart = state.productsToCart.filter((obj: any) => obj.id !== product.id)
